...
type DeleteHostStats struct {
	...
	Metrics            int64 // Number of metrics records deleted
	FilesystemMetrics  int64 // Number of filesystem metrics deleted
...
// DeleteHost removes a host and all associated data from the database.
//
// This function performs a cascade deletion across all related tables:
// - services
// - metrics
// - filesystem_metrics
// - network_metrics
...
// Parameters:
//   - db: Database connection (from InitDB)
//   - hostID: Host identifier to delete
//
// Returns:
//   - *DeleteHostStats: Statistics about deleted records
//   - error: nil if successful, error describing problem if failed
func DeleteHost(db *sql.DB, hostID string) (*DeleteHostStats, error) {
	...
	// Safety check: only allow deletion if host has been offline for > 1 hour
	now := time.Now().Unix()
	secondsSince := now - lastSeen
	oneHour := int64(3600)

	if secondsSince < oneHour {
		return nil, fmt.Errorf("cannot delete host: host was last seen %d seconds ago (must be offline for > 1 hour)", secondsSince)
	}
	...
	// Delete services
	result, err := tx.Exec("DELETE FROM services WHERE host_id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete services: %w", err)
	}
	stats.Services, _ = result.RowsAffected()

	// Delete metrics
	result, err = tx.Exec("DELETE FROM metrics WHERE host_id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete metrics: %w", err)
	}
	stats.Metrics, _ = result.RowsAffected()

	// Delete filesystem_metrics
	result, err = tx.Exec("DELETE FROM filesystem_metrics WHERE host_id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete filesystem_metrics: %w", err)
	}
	stats.FilesystemMetrics, _ = result.RowsAffected()

	// Delete network_metrics
	result, err = tx.Exec("DELETE FROM network_metrics WHERE host_id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete network_metrics: %w", err)
	}
	stats.NetworkMetrics, _ = result.RowsAffected()

	// Delete file_metrics
	result, err = tx.Exec("DELETE FROM file_metrics WHERE host_id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete file_metrics: %w", err)
	}
	stats.FileMetrics, _ = result.RowsAffected()

	// Delete program_metrics
	result, err = tx.Exec("DELETE FROM program_metrics WHERE host_id = ?", hostID)
	if err != nil {
	...
	result, err = tx.Exec("DELETE FROM hosts WHERE id = ?", hostID)
	if err != nil {
		return nil, fmt.Errorf("failed to delete host: %w", err)
	}
	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		return nil, fmt.Errorf("host not found during deletion: %s", hostID)
	}
	...
	log.Printf("[INFO] Deleted host %s and all associated data (services: %d, metrics: %d, events: %d)",
